...
cron on Oracle Cloud daily/weekly.
        ...
        with open(creds_path) as f:
            for line in f:
                if "=" in line:
                    ...
                    elif k == "CHAT_ID": chat = v
    if not token or not chat:
        raise RuntimeError("Telegram credentials missing. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID env vars or use --creds.")
    return token, chat
...
def _conviction_band(score: float) -> str:
    if score >= 9.0: return "high (1.5R)"
    if score >= 7.0: return "baseline (1.0R)"
    if score >= 5.0: return "toehold (0.5R)"
    return "skip"
...
def build_weekly_signal(
    ...
    if regime_json_path and Path(regime_json_path).exists():
        ...
        portfolio_target = {"RISK_ON": 1.0, "RISK_NEU": 0.7, "RISK_OFF": 0.4}.get(regime_state, 1.0)
...
def main():
    ...
    if args.type == "test":
        msg = (f"*V5 system test* — {pd.Timestamp.now()}\n"
               "If you see this, the Telegram pipeline works.\n"
               "_Sent from your Oracle Cloud automation._")
